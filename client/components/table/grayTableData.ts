export default function grayTableData(grayData) {
  ...
  const productTableData = grayData?.products?.reduce((sum, product) => {
    ...
    const leftSideData = `
      ...
      ${tableData(product.chalanId, product?.gray_payments?.length)}
  ...
  const output = productTableData.map((item, index) => {
    if (index === 0) {
      return `
    <tr class="border-b transition-colors  data-[state=selected]:bg-slate-100  dark:data-[state=selected]:bg-slate-800">
  ...
  function tableData(value = "", rowspan) {
    ...
    <td ${
      rowspan ? `rowspan=${rowspan}` : ""
             ...
             <div class="capitalize pl-4">${value}</div>
    </td>`;
  }
